...
const Cart = ({ isOpen, onClose, handleLinkClick, toggleMenu }: CartProps) => {
  ...
  const totalUSD = state.cartItems.reduce(
    (sum, item) => sum + item.price_usd * item.count,
    0,
  )
  const totalUAH = state.cartItems.reduce(
    (sum, item) => sum + item.price_uah * item.count,
    0,
  )
  ...
  return (
    ...
    >
      <DrawerOverlay />
      <DrawerContent bg="white" maxH="100vh" height={drawerHeight} maxW="430px">
        <DrawerBody
        ...
        >
          <Flex justify="space-between" align="center" mb="24px">
            ...
            <Flex justifyContent="flex-end" w="100%">
              <Button onClick={toggleCart} variant="unstyled" display="flex">
                <Image src="/shopping-bag-black.svg" alt="Shopping Bag" />
              </Button>
            </Flex>
          </Flex>

          <Flex justify="space-between" align="center" mb={["16px", "24px"]}>
            <Text m={0} fontSize={["14px", "24px"]} fontWeight={["600", "700"]}>
              {t("Header.cartTitle")}
            </Text>
          ...
          {state.cartItems.length === 0 ? (
            ...
            >
              ...
              <Box pb="2rem" textAlign="center">
                <Text m={0} fontSize="10px" color="#606060" pb="20px">
                  {t("Header.checkoutHint")}
                </Text>
                <Button
                  onClick={() => {
