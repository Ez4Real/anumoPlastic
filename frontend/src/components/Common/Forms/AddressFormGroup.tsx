      ...
      try {
        ...
        if (regions.length > 0) {
          const responses = await Promise.all(
            regions.map((region) =>
              axios.get(
                `https://restcountries.com/v3.1/region/${region}?fields=name,cca2`,
              ),
            ),
          )
          ...
          const response = await axios.get(
            `https://restcountries.com/v3.1${apiEndpoint}?fields=name,cca2`,
          )
        ...
        setCountries(
          countryList
            .filter(
              (country: any) => !countriesToExclude.includes(country.cca2),
            )
            .map((country: any) => ({
              code: country.cca2,
    ...
    <>
      ...
      <FormControl mt="16px">
        <Select
          ...
          placeholder={t(
            "Checkout.delivery.europe.addressFormGroup.placeholders.country",
          )}
          borderRadius={0}
          fontSize="14px"
          borderColor="#A4A2A2"
      ...
      <FormControl mt="12px">
        <Input
          ...
          placeholder={t(
            "Checkout.delivery.europe.addressFormGroup.placeholders.city",
          )}
      ...
      <FormControl mt="12px">
        <Input
          ...
          placeholder={t(
            "Checkout.delivery.europe.addressFormGroup.placeholders.postalCode",
          )}
          border="1px solid #A4A2A2"
          color="#3A3A3A"
          fontSize="14px"
      ...
      <FormControl mt="12px">
        <Input
          ...
          placeholder={t(
            "Checkout.delivery.europe.addressFormGroup.placeholders.deliveryAddress",
          )}
          border="1px solid #A4A2A2"
          color="#3A3A3A"
          fontSize="14px"
