...
import type { OrderPublic } from "../../../../client/models.ts"
...
function getOrderQueryOptions({ id }: { id: string }) {
  return {
    queryFn: () => OrdersService.readOrderById({ id }),
    queryKey: ["order"],
  }
}
...
function Order() {
  ...
  const { data: order, isPending } = useQuery<OrderPublic, Error>({
  ...
  return (
    <Container maxW="full">
      ...
      <Box border="1px solid" borderRadius="12px" mt="1rem">
          ...
          <GridItem>
            ...
            >
              <Flex w="100%" alignItems="center" justifyContent="space-between">
                ...
                <Badge fontSize="lg">{order.currency}</Badge>
              </Flex>
            </VStack>
          </GridItem>
          {/* Customer Info */}
          ...
          <GridItem>
            <VStack align="start" p="1rem" spacing={2}>
              ...
              <Flex w="100%" alignItems="center" justifyContent="space-between">
                ...
                <Text>
                  {t(
                    `AdminPanel.orders.delivery.regions.${order.delivery.region}`,
                  )}
                </Text>
              </Flex>
              <Flex w="100%" alignItems="center" justifyContent="space-between">
                ...
                <Text>
                  {t(`AdminPanel.orders.delivery.types.${order.delivery.type}`)}
                </Text>
              </Flex>
              <Flex w="100%" alignItems="center" justifyContent="space-between">
                <Text
        ...
        {order.delivery.type !== "address" && (
          <Flex w="100%" alignItems="center" px="1rem" gap="2rem">
            <Text
              fontFamily='-apple-system, BlinkMacSystemFont, "Segoe UI"'
              fontSize="14px"
              fontWeight="700"
        ...
        {order.basketOrder.map((item, index) => (
          ...
          >
            ...
            <Box p=".5rem 1rem" maxW="350px">
              ...
              <Text>
                Qty: {item.qty} {item.unit} | Price:{" "}
                {(item.sum / 100).toFixed(2)} {order.currency}
              </Text>
            </Box>
          </Flex>
        ))}
