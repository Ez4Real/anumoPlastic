...
import type { ProductPublic } from "../../../../client/models.ts"
...
function getProductQueryOptions({ id }: { id: string }) {
  return {
    queryFn: () => ProductsService.readProductById({ id }),
    queryKey: ["product"],
  }
}
...
function Product() {
  ...
  const { data: product, isPending } = useQuery<ProductPublic, Error>({
  ...
  return (
    <Container maxW="full" maxH="100vh" overflow="hidden" px={2}>
      <Grid templateColumns={["1fr", "1fr auto 1fr"]} gap={4}>
        <Box
          overflowY="auto"
          maxH="100vh"
          mt={4}
          ...
          <Grid templateColumns={["1fr", "repeat(2, 1fr)"]} gap={3}>
            {product.images?.length ? (
              product.images.map((img, index) => (
                <GridItem h="300px" w="100%" key={index}>
                  <Image
        ...
        <VStack align="start" spacing={4} pt={[0, "1.5rem"]}>
          ...
          <Flex align="center" gap={2}>
            ...
            <Badge borderRadius="4px" fontSize="1em">
              {product.id}
            </Badge>
          </Flex>
          ...
          <Grid w="100%" templateColumns={["1fr", "repeat(2, 1fr)"]} rowGap={4}>
            <Flex align="center" gap={2}>
              <Text
                fontFamily='-apple-system, BlinkMacSystemFont, "Segoe UI"'
                fontSize="14px"
            ...
            <Flex direction="column">
              ...
              <Text fontSize={["14px", "16px"]}>{product.material_en}</Text>
            </Flex>
            <Flex direction="column">
              <Text
                fontFamily='-apple-system, BlinkMacSystemFont, "Segoe UI"'
              ...
              <Text fontSize={["14px", "16px"]}>{product.material_uk}</Text>
            </Flex>
            <Flex direction="column">
              <Text
                fontFamily='-apple-system, BlinkMacSystemFont, "Segoe UI"'
              ...
              <Text fontSize={["14px", "16px"]}>
                {Array.isArray(product.size_en)
                  ? product.size_en.join(", ")
                  : product.size_en}
              </Text>
            </Flex>
            <Flex direction="column">
              ...
              <Text fontSize={["14px", "16px"]}>
                {Array.isArray(product.size_uk)
                  ? product.size_uk.join(", ")
                  : product.size_uk}
              </Text>
            </Flex>
            {product.weight_en && product.weight_uk && (
              <>
                <Flex direction="column">
                  ...
                  <Text fontSize={["14px", "16px"]}>{product.weight_en}</Text>
                </Flex>
                <Flex direction="column">
                  <Text
                    fontFamily='-apple-system, BlinkMacSystemFont, "Segoe UI"'
                  ...
                  <Text fontSize={["14px", "16px"]}>{product.weight_uk}</Text>
                </Flex>
              </>
            )}
          </Grid>
