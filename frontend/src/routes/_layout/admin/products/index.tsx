...
function ProductsTable() {
  ...
  const setPage = (page: number) =>
    navigate({
      search: (prev: { [key: number]: string }) => ({ ...prev, page: page }),
    })
      ...
      >
        ...
        >
          ...
          {isPending ? (
            ...
            <Tbody>
              {products?.data.slice().map((product) => (
                <Tr key={product.id} opacity={isPlaceholderData ? 0.5 : 1}>
                  ...
                  <Td maxWidth="125px">${product.price_usd.toFixed(2)}</Td>
                  <Td maxWidth="125px">₴{product.price_uah.toFixed(2)}</Td>
                </Tr>
              ))}
            </Tbody>
          )}
